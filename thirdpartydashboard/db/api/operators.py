...
from thirdpartydashboard.db.api import base as api_base
...
def operator_get_all(name=None, marker=None, limit=None, sort_field=None, sort_dir=None, **kwargs):
    ...
    query = operator_build_query(operator_name=name, **kwargs)
...
def operator_add_system(operator_id, system_id):
    ...
    with session.begin():
        ...
        system = systems.system_get(system_id, session)
        if system is None:
            raise exc.NotFound("%s %s not found"
                               % ("System", system_id))
